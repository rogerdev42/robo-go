...
func (s *AuthService) SignUp(ctx context.Context, req *models.UserCreate) (*models.User, string, error) {
	s.logger.Info("User signup attempt",
	...
	if existingUser != nil {
		s.logger.Warn("Email already exists", logger.String("email", req.Email))
	...
	if existingUser != nil {
		s.logger.Warn("Username already exists", logger.String("name", req.Name))
		return nil, "", models.ErrAlreadyExists
	}
...
func (s *AuthService) SignIn(ctx context.Context, req *models.UserLogin) (*models.User, string, error) {
	s.logger.Info("User signin attempt", logger.String("email", req.Email))
